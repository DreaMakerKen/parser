			...
			if(rule[i].DerivesEmpty==true){
				...
				for(int j=0,l=0;j<rule[i].fillFollowset.size();j++){
					for(int k=0;k<grammers.size();k++){
						...
						if((rule[k].RHS.contains(rule[i].fillFollowset.get(j)))&&((rule[k].RHS.indexOf(rule[i].fillFollowset.get(j))+1)!=rule[k].RHS.size())){
							for(int m=rule[k].RHS.indexOf(rule[i].fillFollowset.get(j))+1,n=rule[k].RHS.indexOf(rule[i].fillFollowset.get(j))+1; m<rule[k].RHS.size();m++){
								if(isderiveEmpty.contains(rule[k].RHS.get(m))){
									n++;
								}
								if((m==(rule[k].RHS.size()-1))&&(n==(m+1))&&(!rule[k].LHS.equals(rule[i].fillFollowset.get(0)))){
									if(rule[i].fillFollowset.get(j).equals(rule[k].LHS)){
									}
									else{
										if(!rule[i].fillFollowset.contains(rule[k].LHS))	
